...
public class PotraitControl : MonoBehaviour {
    ...
    public void PushStory()
    {
        ...
        if (i == 1)
        {
            NowPo = DataGlobal.GetSubsection(101);
        }
        if (i == 2)
        {
            NowPo = DataGlobal.GetSubsection(102);          
        }
        if (i == 3)
        {
            NowPo = DataGlobal.GetSubsection(103);
        }
        //等待补完
        if (i > 0 && i < 4)
        {
            dialog.text = NowPo.dialogCN;
            Sprite TempSP = new Sprite();
            TempSP = Resources.Load<Sprite>("PotraitSprite/" + NowPo.backGround.ToString()) as Sprite;
            if (TempSP != null)
            {
                backGround.sprite = TempSP;
            }
            Sprite TempPO = new Sprite();
            TempPO = Resources.Load<Sprite>("AVGIdol/" + NowPo.idol.ToString()) as Sprite;
            if (TempPO != null)
            {
                idol.sprite = TempPO;
            }
            backGroundVoice.clip = Resources.Load<AudioClip>("PotraitAudioClip/" + NowPo.audio);
        }
    }

...
public class UI_Control_ScrollFlow : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    private float v = 0;
    private List<UI_Control_ScrollFlow_Item> GotoFirstItems = new List<UI_Control_ScrollFlow_Item>(), GotoLaterItems = new List<UI_Control_ScrollFlow_Item>();
    ...
    public void Check(float _v)
    {
        ...
        {//向左运动
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                {
                    GotoLaterItems.Add(Items[i]);
                }
            }
            if (GotoLaterItems.Count > 0)
            {
                for (int i = 0; i < GotoLaterItems.Count; i++)
                {
                    GotoLaterItems[i].v = Items[Items.Count - 1].v + AddValue;
                    Items.Remove(GotoLaterItems[i]);
                    Items.Add(GotoLaterItems[i]);
                }
                GotoLaterItems.Clear();
            }
        }
        else if (_v > 0)
        {//向右运动，需要把右边的放到前面来
    ...
    public void ToLater(UI_Control_ScrollFlow_Item item)
    {
